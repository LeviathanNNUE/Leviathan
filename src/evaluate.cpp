...
#define S(mg, eg) make_score(mg, eg)
  ...
  constexpr Score BishopOnKingRing    = S( 26,  0);
  ...
  constexpr Score RestrictedPiece     = S(  8,  9);
  constexpr Score RookOnKingRing      = S( 16,  0);
  constexpr Score SliderOnQueen       = S( 62, 21);
  constexpr Score ThreatByKing        = S( 24, 87);
  constexpr Score ThreatByPawnPush    = S( 55, 40);
  constexpr Score ThreatBySafePawn    = S(167, 99);
  constexpr Score TrappedQueen        = S( 20, 10);
  constexpr Score TrappedRook         = S( 55, 15);
...
Value Eval::evaluate(const Position& pos) {
  ...
  {
       int complexity;
       int scale      = 1047 + 11 * pos.non_pawn_material() / 512;
